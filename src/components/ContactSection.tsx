...
const ContactItem: React.FC<{ icon: React.ReactNode; value: string }> = ({ icon, value }) => {
  return (
    <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-4 min-h-14 duration-200 rounded-lg">
      <div className="flex items-center justify-center rounded-lg shrink-0 size-10 transition-colors duration-300">
        {icon}
      </div>
      <p className="text-gray-900 dark:text-gray-100 text-sm sm:text-base font-normal leading-normal flex-1 break-all">
        {value}
      </p>
    </div>
  );
...
const ContactSection: React.FC<ContactSectionProps> = ({ contactInfo }) => {
  ...
  return (
    <section id="contact" className="shrink-0 transition-colors">
      <div className="flex flex-1 justify-center py-5">
        <div className="flex flex-col max-w-4xl flex-1">
          <div className="flex-wrap justify-between gap-3 p-2 sm:p-4">
            <div className="flex min-w-0 flex-col gap-3">
              <p className="text-gray-900 dark:text-gray-100 text-2xl sm:text-3xl font-bold leading-tight tracking-tight">
                Get in Touch
              </p>
